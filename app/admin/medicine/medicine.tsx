...
export default function MedicinePage() {
  ...
  const updateMedicine = (
    ...
    const newMeds: Medicine[] = [...medicines];
    newMeds[index] = {
      ...newMeds[index],
      [field]: value,
    };
  ...
  return (
    <div
      className="p-6 max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6"
      style={{ margin: "auto" }}
    >
      {/* الفورم */}
          ...
          >
            {medicines.map((med, i) => (
              <div key={i} className="flex items-center gap-2 p-1 ">
                <label className="block font-semibold">اسم الدواء</label>
                <input
                  type="text"
                  value={med.name}
                  onChange={(e) =>
      ...
      {/* المعاينة / التقرير */}
      <div id="pdf-content" className="bg-white p-6 rounded shadow">
        <h1
        ...
        <div className="flex flex-col gap-12">
          <div
            className="flex justify-between flex-col gap-7  "
          ...
          <p className="text-lg" style={{ padding: "0 25px" }}>
            اسم التحليل:{" "}
            <span className="font-semibold">{analysis1 || "—"}</span>
          </p>
          <p className="text-lg" style={{ padding: "0 25px" }}>
            اسم التحليل:{" "}
            <span className="font-semibold">{analysis2 || "—"}</span>
          </p>
          <p className="text-lg" style={{ padding: "0 25px" }}>
            ملحوظة مهمة:{" "}
